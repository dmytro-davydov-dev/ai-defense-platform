...
- [x] Scope extension (2026-07-17, per explicit request, immediately following the entry above) — `POST /missions/:id/archive`/`.../unarchive` and `GET /missions?includeArchived`. Delete's DRAFT-only restriction left no way to get a QUEUED/PROCESSING/COMPLETED/FAILED mission out of an operator's default list — archiving fills that gap: `Mission.archivedAt` (migration `20260717150000_mission_archive`), no status restriction, fully reversible, never touches state/audit trail — only `GET /missions`'s default-list visibility. Rationale in [[Mission_State_Machine]]'s "Archiving" section. Converted `MissionsRepository` fully to `$queryRaw`/`$executeRaw` in this pass (previously only `findById`/`findAll`/`softDelete` were raw SQL) — `archivedAt` needs to flow through every returned `MissionRecord`, including from `create`/`updateMetadata`/`updateStatus`/`setVideoObjectKey`, which the stale generated client's return type can't carry; those four still write through the typed `mission` delegate (safer than hand-rolling `UPDATE`/`INSERT` SQL, especially `updateMetadata`'s conditional-field semantics) and then re-read via `findById` — see the repository file's header comment. Unit tests (`missions.service.spec.ts`: `archiveMission`/`unarchiveMission`/`listMissions`) and an e2e round-trip (`mission-lifecycle.e2e-spec.ts`: archive a QUEUED mission, confirm excluded from default list/included with `includeArchived=true`/still reachable by id/audit trail intact, then unarchive) added; e2e not run in this sandbox (same no-docker limitation as REQ-2.14 below). Frontend: `useArchiveMissionMutation`/`useUnarchiveMissionMutation`, `listMissions` query now takes an `includeArchived` arg (`apiSlice.ts`), `ArchiveMissionButton.tsx` (toggle, no confirmation dialog — reversible, non-destructive) wired into `MissionDetailPage.tsx`, `MissionListPage.tsx` gained a "Show archived" switch and an "Archived" chip per row/on the detail header.
